...
function propanel_siteoptions_add_admin() {
    ...
    $tt_page = add_theme_page('تنظیمات پوسته wikivb-birth', 'تنظیمات پوسته wikivb-birth', 'edit_theme_options', 'siteoptions','propanel_siteoptions_options_page');
...
<div class="wrap" id="truethemes_container">
  ...
  <div id="of-popup-reset" class="of-save-popup">
    <div class="of-save-reset">تنظیمات پیش فرض شد</div>
  </div>
  <form action="" enctype="multipart/form-data" id="ofform">
    <div id="header">
      <div class="logo">
        <h2>تنظیمات پوسته wikivb-birth</h2>
      </div>
    ...
    <input type="submit" value="ذخیره‌ی تغییرات" class="button-primary" />
  </form>
  <form action="<?php echo esc_attr( $_SERVER['REQUEST_URI'] ) ?>" method="post" style="display:inline" id="ofform-reset">
    <span class="submit-footer-reset">
    <input name="reset" type="submit" value="بازگشت به حالت پیش فرض" class="button submit-button reset-button" onclick="return confirm('CAUTION: Any and all settings will be lost! Click OK to reset.');" />
...
function propanel_siteoptions_machine($options) {
	...
	foreach ($options as $value) {
		...
		if ( $value['type'] != "heading" ) { 
			...
			$output .= '</div><div class="clear"> </div><div class="explain">'. $explain_value .'</div>'."\n";
